 ...
 */
public class VmAllocationPolicyMigrationEAMASocial extends VmAllocationPolicyAbstract implements VmAllocationPolicyMigration {
    public static final double DEF_UNDERLOAD_THRESHOLD = 0.35;

    /** Custom Metric */
    public int totalHostsOverloaded = 0;

    /** Underloaded Hosts List*/
    public List<Host> underloadedHostsList;
     ...
     */
    public VmAllocationPolicyMigrationEAMASocial(final VmSelectionPolicy vmSelectionPolicy) {
        this(vmSelectionPolicy, null);
    }

    public VmAllocationPolicyMigrationEAMASocial(final VmSelectionPolicy vmSelectionPolicy, final double overUtilization) {
        this(vmSelectionPolicy, null);
        this.setOverUtilizationThreshold(overUtilization);
     ...
     */
    public VmAllocationPolicyMigrationEAMASocial(
        ...
        final BiFunction<VmAllocationPolicy, Vm, Optional<Host>> findHostForVmFunction)
    {
        ...
        this.underloadedHostsList = new ArrayList<>();
    }
    ...
    private Map<Vm, Host> getMigrationMapFromOverloadedHosts(final Set<Host> overloadedHosts) {
        ...
        try{
            ...
            {
                RemOverloadedHosts -= 1;
                List<Vm> tempVmList = overloadedHost.getVmList();
                tempVmList.sort(Comparator.comparingDouble(Vm::getTotalCpuMipsUtilization));
                for(Vm vm: tempVmList){
                    for(Host UPM : underloadedHostsList)
                    {
                        if(!normalList.contains(UPM))
                        {
                            if (!UPM.equals(vm.getHost()) && UPM.isSuitableForVm(vm)){
                                if(((VmSocial)vm).verifyHostSecurity( (HostSocial)UPM) )
                                {
                                    addVmToMigrationMap(migrationMap, vm, UPM);
                                    appendVmMigrationMsgToStringBuilder(builder, vm, UPM);
                                    normalList.add(UPM);
                                }
                            }
                        }
                        if(normalList.size() == underloadedHostsList.size() && RemOverloadedHosts == 0)
                        {
                            break;
                        }
                    }
            ...
            //Algorithm 4
            underloadedHostsList.removeAll(normalList);
            ArrayList<Host> firstHalfULH = new ArrayList<Host>();
            ArrayList<Host> secondHalfULH = new ArrayList<Host>();
            if(underloadedHostsList.size() > 1)
            {
                for(int i=0;i<underloadedHostsList.size();i++){
                    if(i < (underloadedHostsList.size()+1)/2)
                    {
                        firstHalfULH.add(underloadedHostsList.get(i));
                    }
                    else
                        secondHalfULH.add(underloadedHostsList.get(i));
                }
                for (Host FHPM: firstHalfULH){
                    for(Host SHPM: secondHalfULH){
                        var vmListy = SHPM.getVmList();
                        for(Vm vm: vmListy)
                        {
                            if(!normalList.contains(FHPM))
                            {
                                if (!FHPM.equals(vm.getHost()) && FHPM.isSuitableForVm(vm) && isNotHostOverloadedAfterAllocation(FHPM, vm)){
                                    if(((VmSocial)vm).verifyHostSecurity( (HostSocial)FHPM) )
                                    {
                                        addVmToMigrationMap(migrationMap, vm, FHPM);
                                        appendVmMigrationMsgToStringBuilder(builder, vm, FHPM);
                                        normalList.add(FHPM);
                                    }
                                }
                            }
